...
class robot:
  ...
  def __init__(self,name):
    rospy.loginfo("YOLYOYLYOY")
    self.assigned_point=[]
    self.name=name
    self.global_frame=rospy.get_param('~global_frame','/map')
    self.listener=tf.TransformListener()
    rospy.loginfo("YOLYOYLYOY "+name+" "+self.global_frame)
    self.listener.waitForTransform(self.global_frame, name+'/base_link', rospy.Time(0),rospy.Duration(10.0))
    rospy.loginfo("YOLYOYLYOY abdkabskd "+self.name)
    cond=0;	
    while cond==0:	
      try:
    ...
    self.client=actionlib.SimpleActionClient(self.name+'/move_base/move', MoveBaseAction)
